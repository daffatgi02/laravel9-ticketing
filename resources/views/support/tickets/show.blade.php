    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <div>
            @if($ticket->status != 'resolved' && $ticket->status != 'closed' && $ticket->assigned_to_user_id == Auth::id())
                <a href="{{ url('/support/tickets/'.$ticket->id.'/resolve') }}" class="btn btn-success me-2">
                ...
                <div class="card-body">
                    <div class="mb-4">
                        <div class="d-flex align-items-center mb-2">
                            ...
                            <div>
                                <div><strong>{{ $ticket->user->name ?? 'Unknown User' }}</strong></div>
                                <div class="text-muted small">{{ $ticket->created_at->format('M d, Y g:i A') }}</div>
                            </div>
                        </div>
                                    ...
                                    <div class="card-body p-2">
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div>
                                                ...
                                                <div class="text-muted small">{{ round($attachment->size / 1024, 2) }} KB</div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                            ...
                            <div class="chat-message">
                                <div class="d-flex">
                                    ...
                                    <div class="flex-grow-1">
                                        <div class="d-flex justify-content-between align-items-center mb-1">
                                            <div>
                                                ...
                                                <span class="text-muted ms-2">
                                                    {{ $message->user->role == 'it' ? '(IT Support)' :
                                                      ($message->user->role == 'ga' ? '(GA Support)' :
                                                      ($message->user->role == 'hc' ? '(Admin)' : '')) }}
                                                </span>
                                        ...
                                        <div class="chat-bubble {{ Auth::id() == $message->user_id ? 'user' : 'other' }}">
                    ...
                    @if($ticket->status != 'closed' && $ticket->status != 'rejected')
                        ...
                        <form action="{{ url('/support/tickets/'.$ticket->id.'/messages') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="mb-3">
                            ...
                            <div class="mb-3 form-check">
                                ...
                                <label class="form-check-label" for="is_internal">Internal Note (only visible to support staff)</label>
                            </div>
                    ...
                    @else
                        <div class="alert alert-secondary mt-4">
                            <i class="fas fa-lock me-2"></i> This ticket is {{ $ticket->status }}. No new replies can be added.
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    ...
                    <div class="d-flex justify-content-between mb-3">
                        ...
                        <div><strong>{{ $ticket->user->department->name ?? 'N/A' }}</strong></div>
                    </div>
                    ...
                    <div class="d-flex justify-content-between mb-3">
                        <div class="text-muted">Resolved At:</div>
                        <div><strong>{{ \Carbon\Carbon::parse($ticket->resolved_at)->format('M d, Y g:i A') }}</strong></div>
                    </div>
                    @endif
                    ...
                    <div class="d-flex justify-content-between mb-3">
                        ...
                        <div><strong>{{ \Carbon\Carbon::parse($ticket->closed_at)->format('M d, Y g:i A') }}</strong></div>
                    </div>
                    @endif
                </div>
            </div>
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    @if($ticket->status == 'new' || $ticket->status == 'assigned')
                        @if(!$ticket->assigned_to_user_id)
                            <form action="{{ url('/support/tickets/'.$ticket->id.'/take') }}" method="POST" class="mb-3">
                                @csrf
                                <button type="submit" class="btn btn-primary w-100">
                                    <i class="fas fa-user-check me-1"></i> Take Ownership
                                </button>
                            </form>
                        @elseif($ticket->assigned_to_user_id == Auth::id())
                            <form action="{{ url('/support/tickets/'.$ticket->id.'/in-progress') }}" method="POST" class="mb-3">
                                @csrf
                                <button type="submit" class="btn btn-primary w-100">
                    ...
                    @if($ticket->status == 'in_progress' && $ticket->assigned_to_user_id == Auth::id())
                        <form action="{{ url('/support/tickets/'.$ticket->id.'/resolve') }}" method="POST" class="mb-3">
                            @csrf
                    ...
                    @if($ticket->status == 'resolved' && Auth::user()->isAdmin())
                        <form action="{{ url('/admin/tickets/'.$ticket->id.'/close') }}" method="POST" class="mb-3">
                            @csrf
                            @method('PATCH')
                    ...
                    <div class="list-group list-group-flush">
                        ...
                        <div class="list-group-item p-3">
                            ...
                            <div>
                                ...
                                Ticket created by {{ $ticket->user->name }}
                            </div>
                        </div>
                    </div>
                </div>
